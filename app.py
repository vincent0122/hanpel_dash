...
credentials = ServiceAccountCredentials.from_json_keyfile_name(
         'gg.json', scope) # Your json file here
...
##설정값 - 시작#########
need = ['다바오DC','DC BROWN(로버트)', '코코맥스(프랭클린)', '코코맥스(INHILL)(52)','코코맥스(P)','코코맥스30','코코맥스(W50)','페어링파우더(FB)','COCOP','COCOR','코코맥스(ROYAL)','다바오 LOW FAT','맥주효모(중국)','맥주효모(베트남)','대두박','젤라틴(고)','젤라틴(중)','젤라틴(저)','변성타피오카','바나나분말(톤백)','바나나분말(지대)','위너','락토젠','그로우어','미라클','구연산(지대)','솔빈산칼륨(입자)','솔빈산칼륨(가루)','프로피온산칼슘','프로틴파워(소이코밀20)','소이코밀','ISP','멀티락','멀티락(新)','씨센스 프리미엄','디텍','디텍(에이티바이오)','케르세틴','렌틸콩','렌틸콩(지대)','커피화분(허니텍)','인도화분','CLA(유대표님 보관대행)','바이오스플린트(유대표님 보관대행)']
untilWh = 60
        ...
        [
            dcc.Loading(
                ...
                children=[
                    ...
                    dbc.Row(
                        [
                            dbc.Col(html.P("기간과 가중치를 선택하세요"), md=3),
                            dbc.Col(
                                [
                                    dcc.DatePickerRange(
                                        id="date-picker",
                                ...
                                children=[
                                    dcc.Tab(
                                        ...
                                        children=[
                                            dcc.Loading(
                                                id="sales-treemap",
                                                children=[dcc.Graph(figure=salesTree)],
                                                type="default",
                                            )
                                        ],
                                    ),
...
#salesTree.update_traces(marker_colorscale='YlGnBu', selector=dict(type='treemap'))
#salesTree_cus.update_traces(marker_colorscale='YlOrRd', selector=dict(type='treemap'))
#volumeTree.update_traces(marker_colorscale='Rainbow', selector=dict(type='treemap'))
#volumeTree_cus.update_traces(marker_colorscale='RdBu', selector=dict(type='treemap'))
...
BODY = dbc.Container(
    [
        ...
        dbc.Row([dbc.Col(dbc.Card(TREEMAP)),], style={"marginTop": 30}),
    ],
    className="mt-12",
)
